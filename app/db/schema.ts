...
export const images = sqliteTable(
  "images",
  {
    ...
    objectKey: text().notNull().unique(), // Object key for identification
    data: blob({ mode: "buffer" }), // Binary image data (nullable for migration)
    md5: text().notNull().unique(), // MD5 hash for deduplication
    contentType: text().notNull(), // MIME type (image/jpeg, image/png, etc.)
    fileSize: integer().notNull(), // File size in bytes
    uploadedBy: integer().references(() => users.id, { onDelete: "set null" }), // User who uploaded
    createdAt: timestampField(),
    updatedAt: timestampField(),
  },
  (table) => ({
    md5Index: uniqueIndex("images_md5_idx").on(table.md5),
  })
);
